...
func NewOutboxPoller(db *pgxpool.Pool, brokers []string) *OutboxPoller {
	return &OutboxPoller{db: db, writer: &kafka.Writer{Addr: kafka.TCP(brokers...), Balancer: &kafka.LeastBytes{}}, tick: 5 * time.Second}
}
	...
	for {
		...
		case <-time.After(o.tick):
			...
			for rows.Next() {
				...
				if err := o.writer.WriteMessages(ctx, kafka.Message{Topic: topic, Value: payload}); err != nil {
