...
- [ ] **T7.1** `payment-service` `payu_client.go` perbaiki mock → SNAP-BI real (CRITICAL): `payu_client.go:26 Sign` ganti `HMAC(payload+timestamp)` → `stringToSign = method + ":" + endpoint + ":" + token + ":" + hex(sha256(body)) + ":" + timestamp` sesuai `payu/backend/partner-service/src/main/java/id/payu/partner/application/service/SnapBiSignatureService.java`, `CreateTransaction:32-46` hapus `if baseURL==payu-gateway return mock`, implement `1) POST /v1.0/access-token/b2b {X-CLIENT-KEY,X-TIMESTAMP,X-SIGNATURE(rawBody)} → Bearer token` (`SnapBiTokenService:162`), `2) POST /v1.0/transfer-va/payment {Authorization: Bearer token, X-EXTERNAL-ID, X-TIMESTAMP, X-SIGNATURE}` dengan body lengkap `PaymentRequest.java:9-24` (`partnerReferenceNo, amount{value,currency:IDR}, sourceAccountNo, beneficiaryAccountNo, beneficiaryBankCode`), `X-Idempotency-Key` header. `config/config.go` tambah `PayUBaseURL, PayUClientKey, PayUClientSecret, PayUSourceAccount, PayUBeneficiaryAccount`. Verifikasi: `PAYU_BASE_URL=http://payu-partner-service:8080 go test -run TestHMACSignature` → `4012504` hilang, `curl X-Idempotency-Key` 2× replay → `2002500` same `payuReferenceNo` (`SnapBiPaymentService:96`), `payu-partner-service` log `Payment initiated`.
- [ ] **T7.2** `order-service` implementasi minimal Saga (CRITICAL — sekarang scaffold only): `internal/adapter/http/handler.go:8` tambah `POST /v1/orders` `X-Idempotency-Key` wajib → `service.CreateOrder(ctx, userID, items[])` → `SELECT FOR UPDATE` stock via `product-service` `DecrementStock` (`postgres.go:64 WHERE stock >= $2`), `INSERT orders(PENDING) + order_items`, `INSERT outbox(tokobapak.order.created.v1)`; `internal/application/service/service.go:7` implement `CreateOrder, ReserveStock, Compensate(CANCELLED)`, `internal/adapter/postgres/postgres.go:12` implement `Create/UpdateStatus FOR UPDATE`; `cmd/server/main.go:21` wiring `pgxpool + kafka.Writer + OutboxPoller.Start`. Verifikasi: `curl POST /v1/orders {items:[{productId,qty}]} + X-Idempotency-Key → 201 {status:PENDING}` + `GET /v1/products/{id} stock -1`, `2× replay same key → 200 same order`, `stock insufficient → 409 CANCELLED`.
